...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error: errorData.message || errorData.error || 'Failed to fetch projects',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendMessage = (data as { error?: string; message?: string })?.error || (data as { error?: string; message?: string })?.message
    ...
    const responseBody =
      typeof data === 'object' && data !== null
        ? { ...data as object, message: (data as { message?: string }).message || 'Project created successfully' }
        : data
