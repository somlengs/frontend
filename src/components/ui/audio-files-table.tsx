...
export function AudioFilesTable({
  ...
  const isEmpty = files.length === 0 && !loading

  // Calculate skeletons needed - only for initial load
  const skeletonsNeeded = loading ? 5 : 0
          ...
          >
            <table className="w-full caption-bottom text-sm">
              ...
              <tbody>
                {files.map((file, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-sm text-text">{new Date(file.updatedAt || file.createdAt).toLocaleDateString()}</td>
                ...
                {/* Loading Skeletons */}
                {loading && Array.from({ length: skeletonsNeeded }).map((_, i) => (
